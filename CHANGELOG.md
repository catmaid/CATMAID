...
- Different filter types are available at the moment: Only end nodes, only
  ...
  region, created by user(s), binary split, axon, dendrite, synaptic
  connectivity and volumes. Some come with own user input elements, but all can
  optionally be applied to only a subset of skeletons.
