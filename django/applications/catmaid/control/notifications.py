...
def list_notifications(request, project_id = None):
    ...
    change_request_list = [[cr.id, cr.type, cr.status_name(), cr.location.x, cr.location.y, cr.location.z, (cr.treenode if cr.treenode else cr.connector).id, cr.user.get_full_name(), str(cr.creation_time)] for cr in change_requests]
