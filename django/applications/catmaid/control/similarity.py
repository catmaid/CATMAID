...
class PointCloudDetail(APIView):
    ...
    def get(self, request, project_id, config_id):
        ...
        config = PointCloud.objects.get(pk=config_id, project_id=project_id)
        return JsonResponse(serialize_config(config, with_locations, with_images))

    @method_decorator(requires_user_role(UserRole.Annotate))
    def delete(self, request, project_id, pointcloud_id):
        """Delete a point cloud.
        """
        can_edit_or_fail(request.user, config_id, 'pointcloud')
        config = PointCloud.objects.get(pk=pointcloud_id, project_id=project_id)
...
class PointCloudList(APIView):
    ...
    def get(self, request, project_id):
        ...
        """
        simple = get_request_bool(request.query_params, 'simple', False)
        return JsonResponse([serialize_pointcloud(pc, simple) for pc in
                PointCloud.objects.filter(project_id=project_id)], safe=False)

    @method_decorator(requires_user_role(UserRole.Import))
    def put(self, request, project_id):
        """Create a new point cloud by providing the complete set of points.
        ---
        parameters:
          - name: project_id
            description: Project of the point cloud
            type: integer
            paramType: path
            required: true
          - name: name
            ...
            required: true
          - name: description
            description: Description of the point cloud
            type: string
            paramType: form
            required: true
          - name: locations
            description: Points as a list of XYZ-lists
            required: true
            paramType: form
          - name: source_path
            description: A source reference of the point cloud
            required: false
            paramType: form
        """
        name = request.data.get('name')
