...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.execute('DROP INDEX IF EXISTS connector_x_index')
        db.execute('DROP INDEX IF EXISTS connector_y_index')
        db.execute('DROP INDEX IF EXISTS connector_z_index')

        db.execute('DROP INDEX IF EXISTS location_x_index')
        db.execute('DROP INDEX IF EXISTS location_y_index')
        db.execute('DROP INDEX IF EXISTS location_z_index')

        statement_fmt = 'CREATE INDEX %s_location_%s_index ON %s USING btree (((location).%s));'
        for table in ('treenode', 'connector', 'location'):
            for dimension in ('x', 'y', 'z'):
                db.execute(statement_fmt % (table, dimension, table, dimension))        

        db.create_index('treenode', ['parent_id'])

        db.create_index('class_instance', ['user_id'])
        db.create_index('class_instance', ['project_id'])
        db.create_index('class_instance', ['class_id'])

        for column in ['user_id', 'project_id', 'relation_id', 'class_instance_a', 'class_instance_b']:
            db.create_index('class_instance_class_instance', [column])

        for column in ['user_id', 'project_id']:
            db.create_index('class', [column])

        for column in ['user_id', 'project_id', 'relation_id', 'class_a', 'class_b']:
            db.create_index('class_class', [column])

        for column in ['user_id', 'project_id', 'relation_id', 'class_instance_id']:
            db.create_index('treenode_class_instance', [column])

        for column in ['user_id', 'project_id', 'relation_id', 'class_instance_id']:
            db.create_index('connector_class_instance', [column])

        for column in ['user_id', 'project_id', 'relation_id', 'treenode_id', 'connector_id', 'skeleton_id']:
            db.create_index('treenode_connector', [column])
