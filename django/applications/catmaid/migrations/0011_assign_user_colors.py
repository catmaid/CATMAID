...
from catmaid.control.user import initial_colors
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        i = 0
        for user_profile in orm.UserProfile.objects.exclude(user_id__exact=-1).order_by('user'):
            if i < len(initial_colors):
                rgba = initial_colors[i]
            else:
                rgba = colorsys.hsv_to_rgb(random.random(), random.random(), 1.0) + (1,)
            user_profile.color = RGBA(rgba[0], rgba[1], rgba[2], rgba[3])
            user_profile.save()
            i += 1
    ...
    models = {
        ...
        'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(1.0, 0.4260732757199084, 0.1218274170610214, 1)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
