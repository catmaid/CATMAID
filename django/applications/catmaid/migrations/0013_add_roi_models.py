...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'RegionOfInterest'
        db.execute('''CREATE TABLE region_of_interest (
                    stack_id integer REFERENCES stack(id),
                    zoom_level integer NOT NULL,
                    width double precision NOT NULL,
                    height double precision NOT NULL,
                    rotation_cw double precision NOT NULL)
                    INHERITS (location)''')
        db.execute('''ALTER TABLE only region_of_interest
                    ADD CONSTRAINT region_of_interest_pkey PRIMARY KEY (id)''');
        db.send_create_signal('catmaid', ['RegionOfInterest'])

        # Adding model 'RegionOfInterestClassInstance'
        db.execute('''CREATE TABLE region_of_interest_class_instance (
                    region_of_interest_id integer REFERENCES region_of_interest(id),
                    class_instance_id integer REFERENCES class_instance(id))
                    INHERITS (relation_instance)''')
        db.execute('''ALTER TABLE only region_of_interest_class_instance
                    ADD CONSTRAINT region_of_interest_class_instance_pkey PRIMARY KEY (id)''');
        db.send_create_signal('catmaid', ['RegionOfInterestClassInstance'])


    def backwards(self, orm):
        # Deleting model 'RegionOfInterest'
        db.delete_table('region_of_interest')

        # Deleting model 'RegionOfInterestClassInstance'
        db.delete_table('region_of_interest_class_instance')
    ...
    models = {
        ...
        'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0.5, 0, 1, 1)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
