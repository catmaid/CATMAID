...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ This migration looks at the skeletonlist_dashboard table to convert
        it into an annoation of all it's entries. After this has been done
        successfully, the table will be removed. There is no going back for this
        ...
        while answer not in ['yes', 'no', 'skip']:
            answer = raw_input("This migration will create new data " \
                "(annotations) in the database, based on existing skeleton " \
                "lists. It cannot be reversed. Please make sure you have an " \
                "up-to-date backup. Only data related to the skeleton lists " \
                "will be changed. Do you want to continue? (yes/no) ")
        if answer != 'yes':
        ...
        for sl in skeletonlists:
            ...
            annotation_class, created = orm.Class.objects.get_or_create(
                    ...
                    defaults={'user_id': super_user.id})
            if created:
                print("Created annotation class for project #%s" % \
                        sl.project.id)

            # Get or create the annotated_with realation
            annotated_with_rel, created = orm.Relation.objects.get_or_create(
                    project_id=sl.project.id, relation_name='annotated_with',
                    defaults={'user_id': super_user.id})
            if created:
                print("Created annotated_with relation for project #%s" % \
                        sl.project.id)

            # Get the annotation for the the current list
            annotation, created = orm.ClassInstance.objects.get_or_create(
                    project_id=sl.project.id, name=sl.shortname,
                    class_column_id=annotation_class.id,
                    defaults={'user_id': super_user.id})
            if created:
                print("Created annotation '%s' for project #%s" % \
                        (sl.shortname, sl.project.id))

            # Link this new annotation to all skeleton the list refers to
            for skid in sl.skeleton_list:
                try:
                    # Get neuron that is modeled by this skeleton
                    neuron = orm.ClassInstance.objects.get(
                            cici_via_b__relation__relation_name='model_of',
                            cici_via_b__class_instance_a=skid,
                            project=sl.project.id)
                except orm.ClassInstance.DoesNotExist:
                    print("Could not find neuron for skeleton #%s, ignoring it." \
                            % skid)
                    continue

                # Link annotation to neuron of skeleton
                cici, created = orm.ClassInstanceClassInstance.objects.get_or_create(
                        project=sl.project, relation=annotated_with_rel,
                        class_instance_a_id=neuron.id,
                        class_instance_b_id=annotation.id,
                        defaults={
                            'user_id': sl.user.id,
                        })
                cici.save() # update the last edited time
                if created:
                    print("Linked annotation '%s' to neuron #%s" % \
                            (annotation.name, neuron.id))
                else:
                    print("Link from annotation '%s' to neuron #%s exists already" % \
                            (annotation.name, neuron.id))

        print("Done with the data migrating")
    ...
    models = {
        ...
        'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0.6068165337642772, 0.6363124656797958, 1.0, 1)'}),
