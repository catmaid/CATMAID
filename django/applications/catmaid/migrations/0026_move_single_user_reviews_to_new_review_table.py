...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Every treenode can currently be reviewed by one user only. This data
        migration will go though all treenode reviews and create a new entry for
        each one in the new review table created in migration 0026. The data is
        essentially duplicated. After all the tools have been adjusted to use
        the new review system, the current review representation will be removed.
        """
        # Un-reviewed treenodes have a reviewer ID of -1.
        # Get only the treenodes that have been reviewed.
        reviewed_treenodes = orm.Treenode.objects.exclude(reviewer_id=-1) \
                .order_by("skeleton")
        nr_treenodes = reviewed_treenodes.count()

        # Calculate percentage output marks for output every 10%
        output_count = {}
        for i in range(10):
            output_count[i*(nr_treenodes/10)] = i * 10
        output_count[nr_treenodes - 1] = 100

        # Go though all treenodes and create a new entry in the review table for
        # each node. This has to be done *without* the cache, because the
        # treenode table can get very big. Therefore, iterator() is used.
        for i, tn in enumerate(reviewed_treenodes.iterator()):
            # Give some rough estimate were we are
            if i in output_count:
                sys.stdout.write("   %s%%" % output_count[i])
                sys.stdout.flush()
            # Create the new entry
            orm.Review.objects.create(
                project_id=tn.project_id,
                reviewer_id=tn.reviewer_id,
                review_time=tn.review_time,
                treenode_id=tn.id,
                skeleton_id=tn.skeleton_id)

        # Write a final new line, if some output was made before
        if nr_treenodes > 0:
            sys.stdout.write("\n")

    def backwards(self, orm):
        """ This backwards method walks through the review table and overwrites
        the review fields of the corresponding treenode table entry. If there is
        more than one entry, the newest is used.
        """
        reviews = orm.Review.objects.order_by('-review_time')
        nr_reviews = reviews.count()

        # Calculate percentage output marks for output every 10%
        output_count = {}
        for i in range(10):
            output_count[i*(nr_reviews/10)] = i * 10
        output_count[nr_reviews - 1] = 100
        # Go though all reviews and override the review information for the
        # treenode referred to. The newest on wins. This has to be done
        # *without* the cache, because the treenode table can get very big.
        # Therefore, iterator() is used.
        for i, r in enumerate(reviews.iterator()):
            # Give some rough estimate were we are
            if i in output_count:
                sys.stdout.write("   %s%%" % output_count[i])
                sys.stdout.flush()
            # Write single-user review
            orm.Treenode.objects.filter(pk=r.treenode_id).update(
                    reviewer_id=r.reviewer_id,
                    review_time=r.review_time)
            # Delete multi-user review
            r.delete()
    ...
    models = {
        ...
        u'catmaid.relation': {
            ...
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'isreciprocal': ('django.db.models.fields.BooleanField', [], {}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.Project']"}),
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(1.0, 0.3528471185921418, 0.9575391320987333, 1)'}),
