...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Cut inheritance link from location to concept
        db.execute("ALTER TABLE location NO INHERIT concept;")

        # Create own nextval() trigger for the ID fields of all location sub-classes
        db.execute("ALTER TABLE location ALTER COLUMN id DROP DEFAULT;")
        db.execute("""CREATE SEQUENCE location_id_seq
                   START WITH 1
                   INCREMENT BY 1
                   NO MINVALUE
                   NO MAXVALUE
        ...
        db.execute("SELECT setval('location_id_seq', (SELECT MAX(id) FROM location));");

        # Make the sequence the default for all location based ID columns
        db.execute("ALTER TABLE location ALTER COLUMN id SET DEFAULT nextval('location_id_seq'::regclass);")


    def backwards(self, orm):
        # Remove default sequence of location.id and remove it
        db.execute("ALTER TABLE location ALTER COLUMN id DROP DEFAULT;")
        db.execute("DROP SEQUENCE location_id_seq;")

        # Make location inherit concept
        db.execute("ALTER TABLE location INHERIT concept;")

        # Add default sequence to all location based relations
        db.execute("ALTER TABLE location ALTER COLUMN id SET DEFAULT nextval('concept_id_seq'::regclass);");
    ...
    models = {
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(1.0, 0.8676086276856803, 0.33570885993731825, 1)'}),
