...
tables = ('location', 'treenode', 'region_of_interest', 'connector')
columns = ('creation_time', 'edition_time')


class Migration(SchemaMigration):

    def create_index_name(self, table, column):
        return "%s_%s_index" % (table, column)

    def create_index(self, table, column):
        index_name = self.create_index_name(table, column)
        db.execute('''
            DO $$
            BEGIN
    ...
    def forwards(self, orm):
        for t in tables:
            for c in columns:
                self.create_index(t, c)


    def backwards(self, orm):
        for t in tables:
            for c in columns:
                db.execute('DROP INDEX %s' % self.create_index_name(t, c))
    ...
    models = {
        ...
        u'catmaid.changerequest': {
            ...
            'creation_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'db_index': 'True'}),
            'description': ('django.db.models.fields.TextField', [], {}),
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'db_index': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0, 0, 1, 1)'}),
