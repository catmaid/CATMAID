...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Fix a performance regression in the trigger function for updating
        # treenodes. IS NOT DICTINCT FROM on a primary key results in a
        # sequential scan rather than an index. Since only the queried value can
        # be null, not the key itself, simplify this to a NULL check and an
        # index scan.
        db.execute('''
            CREATE OR REPLACE FUNCTION on_edit_treenode_update_edges() RETURNS trigger\n
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
            ...
            if OLD.parent_id IS DISTINCT FROM NEW.parent_id OR
                ...
                INSERT INTO treenode_edge (id, project_id, edge) (
                    SELECT NEW.id, NEW.project_id, ST_MakeLine(
                        ...
                        ST_MakePoint(p.location_x, p.location_y, p.location_z))
                    FROM treenode p
                    WHERE (p.id IS NOT DISTINCT FROM NEW.parent_id) OR
                          (NEW.parent_id IS NULL AND NEW.id = p.id));
            end if;
            RETURN NEW;
            END;
    ...
    models = {
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0.9350996627281424, 0.9137501804888306, 1.0, 1)'}),
