...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'StackClassInstance'
        db.execute('''CREATE TABLE stack_class_instance (
                   stack_id integer REFERENCES stack (id),
                   class_instance_id integer REFERENCES class_instance (id))
                   INHERITS (relation_instance)''')

        db.send_create_signal(u'catmaid', ['StackClassInstance'])


    def backwards(self, orm):
        # Deleting model 'StackClassInstance'
        db.delete_table('stack_class_instance')
