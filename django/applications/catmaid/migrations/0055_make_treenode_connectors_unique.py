...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Update duplicate treenode connectors with a new edition time.
        db.execute('''
                WITH summary AS (
                    SELECT tc.id,
                           tc.project_id,
                           ...
                           ROW_NUMBER() OVER(
                               PARTITION BY tc.project_id,
                                            ...
                                            tc.connector_id,
                                            tc.relation_id
                               ORDER BY tc.creation_time ASC) AS rk
                      FROM treenode_connector tc
                      WHERE (tc.project_id, tc.treenode_id, tc.connector_id, tc.relation_id) IN (
                          SELECT project_id, treenode_id, connector_id, relation_id
                            FROM treenode_connector
                            GROUP BY project_id, treenode_id, connector_id, relation_id
                            HAVING count(*) > 1))
                UPDATE treenode_connector
                  SET edition_time = NOW()
                  FROM summary s
                  WHERE treenode_connector.id = s.id
                    AND s.rk = 1
                ''')

        # Delete duplicate treenode connectors that do not have the oldest
        # creation time.
        db.execute('''
                WITH summary AS (
                    SELECT tc.id,
                      ...
                      WHERE (tc.project_id, tc.treenode_id, tc.connector_id, tc.relation_id) IN (
                          SELECT project_id, treenode_id, connector_id, relation_id
                            ...
                            GROUP BY project_id, treenode_id, connector_id, relation_id
                            HAVING count(*) > 1))
                DELETE FROM treenode_connector
                  USING summary s
                  WHERE treenode_connector.id = s.id
                    AND s.rk > 1
                ''')
        # Adding unique constraint on 'TreenodeConnector', fields ['project', 'treenode', 'connector', 'relation']
        db.create_unique('treenode_connector', ['project_id', 'treenode_id', 'connector_id', 'relation_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'TreenodeConnector', fields ['project', 'treenode', 'connector', 'relation']
        db.delete_unique('treenode_connector', ['project_id', 'treenode_id', 'connector_id', 'relation_id'])
    ...
    models = {
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0.18687787577716486, 0.6819129859376694, 1.0, 1)'}),
