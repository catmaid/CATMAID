...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def make_settings_entry(key, value):
            return '"%s":{"overridable":true,"value":%s}' % (key, value)

        def store_default_settings(settings_datastore, key, entries):
            present_entries = [e for e in entries if hasattr(settings, e['cm_name'])]

            if not present_entries:
                return

            entries_json = [make_settings_entry(e['datastore_name'],
                                                e['serializer'](getattr(settings, e['cm_name']))) for e in present_entries]
            value = '{"version":0,"entries":{%s}}' % ','.join(entries_json)
            datastore_settings = orm.ClientData.objects.get_or_create(
                    datastore=settings_datastore,
                    key=key,
                    project=None,
                    user=None)[0]
            datastore_settings.value = value
            datastore_settings.save()

        def store_user_settings(settings_datastore, profile, key, entries):
            entries_json = [make_settings_entry(e['datastore_name'],
                                                e['serializer'](getattr(profile, e['profile_name']))) for e in entries]
            value = '{"version":0,"entries":{%s}}' % ','.join(entries_json)
            datastore_settings = orm.ClientData.objects.get_or_create(
                    datastore=settings_datastore,
                    key=key,
        ...
        settings_datastore = orm.ClientDatastore.objects.get_or_create(name='settings')[0]

        store_default_settings(
                settings_datastore,
                'navigator',
                [{'datastore_name': 'invert_mouse_wheel',
                  'cm_name': 'PROFILE_DEFAULT_INVERSE_MOUSE_WHEEL',
                  'serializer': lambda x: 'true' if x else 'false'},
                 {'datastore_name': 'use_cursor_following_zoom',
                  'cm_name': 'PROFILE_USE_CURSOR_FOLLOWING_ZOOM',
                  'serializer': lambda x: 'true' if x else 'false'}])

        store_default_settings(
                settings_datastore,
                'stack-viewer',
                [{'datastore_name': 'display_stack_reference_lines',
                  'cm_name': 'PROFILE_DISPLAY_STACK_REFERENCE_LINES',
                  'serializer': lambda x: 'true' if x else 'false'}])

        store_default_settings(
                settings_datastore,
                'tile-layer',
                [{'datastore_name': 'prefer_webgl',
                  'cm_name': 'PROFILE_PREFER_WEBGL_LAYERS',
                  'serializer': lambda x: 'true' if x else 'false'},
                 {'datastore_name': 'linear_interpolation',
                  'cm_name': 'PROFILE_TILE_LINEAR_INTERPOLATION',
                  'serializer': lambda x: 'true' if x else 'false'}])

        store_default_settings(
                settings_datastore,
                'tracing-overlay',
                [{'datastore_name': 'screen_scaling',
                  'cm_name': 'PROFILE_TRACING_OVERLAY_SCREEN_SCALING',
                  'serializer': lambda x: 'true' if x else 'false'},
                 {'datastore_name': 'scale',
                  'cm_name': 'PROFILE_TRACING_OVERLAY_SCALE',
                  'serializer': str}])
        ...
        for profile in orm.UserProfile.objects.all():
            store_user_settings(
                    ...
                    'navigator',
                    [{'datastore_name': 'invert_mouse_wheel',
                      'profile_name': 'inverse_mouse_wheel',
                      'serializer': lambda x: 'true' if x else 'false'},
                     {'datastore_name': 'use_cursor_following_zoom',
                      'profile_name': 'use_cursor_following_zoom',
                      'serializer': lambda x: 'true' if x else 'false'}])
            ...
            store_user_settings(
                    ...
                    'stack-viewer',
                    [{'datastore_name': 'display_stack_reference_lines',
                      'profile_name': 'display_stack_reference_lines',
                      'serializer': lambda x: 'true' if x else 'false'}])
            ...
            store_user_settings(
                    ...
                    'tile-layer',
                    [{'datastore_name': 'prefer_webgl',
                      'profile_name': 'prefer_webgl_layers',
                      'serializer': lambda x: 'true' if x else 'false'},
                     {'datastore_name': 'linear_interpolation',
                      'profile_name': 'tile_linear_interpolation',
                      'serializer': lambda x: 'true' if x else 'false'}])
            ...
            store_user_settings(
                    ...
                    'tracing-overlay',
                    [{'datastore_name': 'screen_scaling',
                      'profile_name': 'tracing_overlay_screen_scaling',
                      'serializer': lambda x: 'true' if x else 'false'},
                     {'datastore_name': 'scale',
                      'profile_name': 'tracing_overlay_scale',
                      'serializer': str}])
    ...
    models = {
        ...
        u'catmaid.clientdata': {
            'Meta': {'unique_together': "(('datastore', 'key', 'project', 'user'),)", 'object_name': 'ClientData', 'db_table': "'client_data'"},
            'datastore': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.ClientDatastore']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'key': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.Project']", 'null': 'True', 'blank': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']", 'null': 'True', 'blank': 'True'}),
            'value': ('jsonfield.fields.JSONField', [], {'default': '{}'})
        },
        u'catmaid.clientdatastore': {
            'Meta': {'object_name': 'ClientDatastore', 'db_table': "'client_datastore'"},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'catmaid.stackclassinstance': {
            'Meta': {'object_name': 'StackClassInstance', 'db_table': "'stack_class_instance'"},
            'class_instance': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.ClassInstance']"}),
            'creation_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'catmaid.suppressedvirtualtreenode': {
            'Meta': {'object_name': 'SuppressedVirtualTreenode', 'db_table': "'suppressed_virtual_treenode'"},
            'child': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.Treenode']"}),
            'creation_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'location_coordinate': ('django.db.models.fields.FloatField', [], {}),
            'orientation': ('django.db.models.fields.SmallIntegerField', [], {}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.Project']"}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']"})
        },
        u'catmaid.textlabel': {
        ...
        u'catmaid.treenodeconnector': {
            'Meta': {'unique_together': "(('project', 'treenode', 'connector', 'relation'),)", 'object_name': 'TreenodeConnector', 'db_table': "'treenode_connector'"},
            'confidence': ('django.db.models.fields.IntegerField', [], {'default': '5'}),
            'connector': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['catmaid.Connector']"}),
            'creation_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(0.2736828001331879, 1.0, 0.4926583080448991, 1)'}),
            ...
            'tile_linear_interpolation': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
            'tracing_overlay_scale': ('django.db.models.fields.FloatField', [], {'default': '1.0'}),
        ...
        u'catmaid.volume': {
            ...
            'creation_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'edition_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'editor': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'editor'", 'db_column': "'editor_id'", 'to': u"orm['auth.User']"}),
