...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.execute('''
            CREATE OR REPLACE FUNCTION
              check_treenode_connector_related_reviews(tc treenode_connector)
              RETURNS void
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
        ...
        db.execute('''
            CREATE OR REPLACE FUNCTION on_create_treenode_connector_check_review() RETURNS trigger\n
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
            -- Mark linked treenodes as unreviewed. If relation is postsynaptic,
            -- mark only one, otherwise mark all treenodes related to connector.
            ...
            $$;''')
        db.execute('''
            CREATE TRIGGER on_create_treenode_connector_check_review
            AFTER INSERT ON treenode_connector
            FOR EACH ROW EXECUTE PROCEDURE on_create_treenode_connector_check_review()''')

        db.execute('''
            CREATE OR REPLACE FUNCTION on_edit_treenode_connector_check_review() RETURNS trigger\n
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
            ...
            $$;''')
        db.execute('''
            CREATE TRIGGER on_edit_treenode_connector_check_review
            AFTER UPDATE ON treenode_connector
            FOR EACH ROW EXECUTE PROCEDURE on_edit_treenode_connector_check_review()''')

        db.execute('''
            CREATE OR REPLACE FUNCTION on_delete_treenode_connector_check_review() RETURNS trigger\n
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
            -- Mark linked treenodes as unreviewed. If relation is postsynaptic,
            -- mark only one, otherwise mark all treenodes related to connector.
            ...
            $$;''')
        db.execute('''
            CREATE TRIGGER on_delete_treenode_connector_check_review
            AFTER DELETE ON treenode_connector
            FOR EACH ROW EXECUTE PROCEDURE on_delete_treenode_connector_check_review()''')

        db.execute('''
            CREATE OR REPLACE FUNCTION on_edit_connector_check_review() RETURNS trigger\n
            LANGUAGE plpgsql\n
            AS $$BEGIN\n
            ...
            $$;''')
        db.execute('''
            CREATE TRIGGER on_edit_connector_check_review
            AFTER UPDATE ON connector
            FOR EACH ROW EXECUTE PROCEDURE on_edit_connector_check_review()''')

    def backwards(self, orm):
        db.execute('DROP TRIGGER on_edit_connector_check_review ON connector')
        db.execute('DROP FUNCTION on_edit_connector_check_review()')

        db.execute('DROP TRIGGER on_delete_treenode_connector_check_review ON treenode_connector')
        db.execute('DROP FUNCTION on_delete_treenode_connector_check_review()')

        db.execute('DROP TRIGGER on_edit_treenode_connector_check_review ON treenode_connector')
        db.execute('DROP FUNCTION on_edit_treenode_connector_check_review()')

        db.execute('DROP TRIGGER on_create_treenode_connector_check_review ON treenode_connector')
        db.execute('DROP FUNCTION on_create_treenode_connector_check_review()')

        db.execute('DROP FUNCTION check_treenode_connector_related_reviews(treenode_connector)')
    ...
    models = {
        ...
        u'catmaid.userprofile': {
            ...
            'color': ('catmaid.fields.RGBAField', [], {'default': '(1.0, 0.11908388130884939, 0.686715197713232, 1)'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
