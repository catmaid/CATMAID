...
forward = """
    CREATE OR REPLACE FUNCTION refresh_skeleton_summary_table_for_project(target_project_id int) RETURNS void
    LANGUAGE plpgsql AS
    $$
    BEGIN
        -- Disable all indexes
        UPDATE pg_index set indisvalid=false, indisready=false
        WHERE indrelid = format('catmaid_skeleton_summary__%s', target_project_id::text)::regclass;

        -- Remove all entries
        DELETE FROM catmaid_skeleton_summary where project_id = target_project_id;
        ...
        WITH node_data AS (
            ...
            (
              ...
              FROM treenode WHERE project_id = target_project_id
            ) edit
            JOIN
            (
            ...
            JOIN
            (
              SELECT skeleton_id, COUNT(*) AS nodes FROM treenode
            ...
            (
              ...
              WHERE t1.project_id = target_project_id
                AND t2.project_id = target_project_id
        ...
        -- Re-enable all indexes and re-index
        UPDATE pg_index set indisvalid=true, indisready=true, indislive=true
        WHERE indrelid = format('catmaid_skeleton_summary__%s', target_project_id::text)::regclass;

        EXECUTE format('REINDEX TABLE %I', format('catmaid_skeleton_summary__%s', target_project_id::text));
    END;
    $$;
"""
...
class Migration(migrations.Migration):
    """
    Update the summary refresh functions to disable indexes during the update
    This requires ownership, which we can assume.
    """

    dependencies = [
        ('catmaid', '0125_partition_skeleton_summary_table_by_project'),
    ]
