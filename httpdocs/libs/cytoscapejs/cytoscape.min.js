...
/**
 * This file is part of cytoscape.js 2.0.0beta2-github-snapshot-2013.02.10-21.14.34.
 * 
...
}else{if(h.number){var e=g.valueMin+(g.valueMax-g.valueMin)*d;i=this.parse(g.name,e,g.bypass)}else{return false}}if(!i){return false}i.mapping=g;g=i;break;case a.style.types.data:l=n._private.data[g.field];i=this.parse(g.name,l,g.bypass);if(!i){return false}i.mapping=g;g=i;break;case undefined:break;default:return false}if(m){if(p){g.bypassed=u.bypassed}else{g.bypassed=u}t[g.name]=g}else{if(p){u.bypassed=g}else{t[g.name]=g}}return true};a.styfn.applyProperty=function(f,d,e){var c=this.parse(d,e);
...
return}this.length=1;this[0]=this;this._private={cy:d,single:true,data:f.data||{},position:f.position||{},listeners:[],group:f.group,style:{},removed:true,selected:f.selected?true:false,selectable:f.selectable===undefined?true:(f.selectable?true:false),locked:f.locked?true:false,grabbed:false,grabbable:f.grabbable===undefined?true:(f.grabbable?true:false),classes:{},animation:{current:[],queue:[]},rscratch:{},scratch:{},edges:[],children:[]};if(f.renderedPosition){var o=f.renderedPosition;var j=d.pan();
var m=d.zoom();this._private.position={x:(o.x-j.x)/m,y:(o.y-j.y)/m}}if(b.is.string(f.classes)){var e=f.classes.split(/\s+/);for(var g=0,c=e.length;g<c;g++){var k=e[g];if(!k||k===""){continue}n._private.classes[k]=true}}if(h===undefined||h){this.restore()}};b.Collection=function(h,c){if(!(this instanceof b.Collection)){return new b.Collection(h,c)}if(h===undefined||!b.is.core(h)){b.util.error("A collection must have a reference to the core");return}var d={};var f=[];var o=false;if(!c){c=[]}else{if(c.length>0&&b.is.plainObject(c[0])&&!b.is.element(c[0])){o=true;
var s=[];var j={};for(var n=0,g=c.length;n<g;n++){var p=c[n];if(p.data==null){p.data={}}var m=p.data;if(m.id==null){m.id=a.generate(h,p)}else{if(h.getElementById(m.id).length!=0||j[m.id]){continue}}var r=new b.Element(h,p,false);s.push(r);j[m.id]=true}c=s}}for(var n=0,g=c.length;n<g;n++){var k=c[n];if(!k){continue}var e=k._private.data.id;if(!d[e]){d[e]=k;f.push(k)}}for(var n=0,g=f.length;n<g;n++){this[n]=f[n]}this.length=f.length;this._private={cy:h,ids:d};if(o){this.restore()}};b.elefn=b.elesfn=b.Element.prototype=b.Collection.prototype;
b.elesfn.cy=function(){return this._private.cy};b.elesfn.element=function(){return this[0]};b.elesfn.collection=function(){if(b.is.collection(this)){return this}else{return new b.Collection(this._private.cy,[this])}};b.elesfn.json=function(){var g=this.element();if(g==null){return undefined}var h=g._private;var f=b.util.copy({data:h.data,position:h.position,group:h.group,bypass:h.bypass,removed:h.removed,selected:h.selected,selectable:h.selectable,locked:h.locked,grabbed:h.grabbed,grabbable:h.grabbable,classes:"",scratch:h.scratch});
...
this.updateEdgesCache()}if(i.type=="load"){this.load()}if(i.type=="viewport"){this.data.canvasNeedsRedraw[w]=true;this.data.canvasRedrawReason[w].push("viewchange");this.data.canvasNeedsRedraw[d]=true;this.data.canvasRedrawReason[d].push("viewchange");this.data.canvasNeedsRedraw[z]=true;this.data.canvasRedrawReason[z].push("viewchange")}this.data.canvasNeedsRedraw[d]=true;this.data.canvasRedrawReason[d].push("notify");this.data.canvasNeedsRedraw[z]=true;this.data.canvasRedrawReason[z].push("notify");
this.redraws++;this.redraw()};t.prototype.load=function(){var i=this;i.data.container.addEventListener("mousedown",function(L){i.hoverData.capture=true;var F=i.data.cy;var N=i.projectIntoViewport(L.pageX,L.pageY);var O=i.data.select;var K=i.findNearestElement(N[0],N[1]);var P=i.hoverData.down;var M=i.dragData.possibleDragElements;if(L.button==0){if(K&&K._private.grabbable){if(K._private.group=="nodes"&&K._private.selected==false){K._private.grabbed=true;K.trigger(new s.Event(L,{type:"grab"}));var E=new s.Event(L,{type:"unselect"});
var D=new s.Event(L,{type:"free"});for(var I=0;I<M.length;I++){var G=M[I];var Q=false;if(G._private.selected||G._private.grabbed){Q=true}if(G._private.selected){G._private.selected=false;G.trigger(E)}if(G._private.grabbed){G._private.grabbed=false;G.trigger(D)}if(Q){G.updateStyle(false)}}i.dragData.possibleDragElements=M=[];M.push(K);for(var I=0;I<K._private.edges.length;I++){K._private.edges[I]._private.grabbed=true}}if(K._private.group=="nodes"&&K._private.selected==true){var B=new s.Event(L,{type:"grab"});
for(var I=0;I<M.length;I++){if(M[I]._private.group=="nodes"){M[I]._private.grabbed=true;var C=M[I]._private.edges;for(var H=0;H<C.length;H++){C[H]._private.grabbed=true}M[I].trigger(B)}}}K.trigger(new s.Event(L,{type:"mousedown"}));i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Single node moved to drag layer");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Single node moved to drag layer")}else{if(K==null){var J=new s.Event(L,{type:"mousedown"});F.trigger(J)
}}i.hoverData.down=K;i.hoverData.downTime=(new Date()).getTime();if(K==null){O[4]=1}}else{if(L.button==1){i.hoverData.dragging=true;i.hoverData.initialPan=[F.pan().x,F.pan().y]}}O[0]=O[2]=N[0];O[1]=O[3]=N[1];i.redraw()},false);window.addEventListener("mousemove",function(I){var D=i.data.cy;var K=i.projectIntoViewport(I.pageX,I.pageY);var L=i.data.select;var H=i.findNearestElement(K[0],K[1]);var N=i.hoverData.last;var M=i.hoverData.down;var O=[K[0]-L[2],K[1]-L[3]];var C=i.getCachedNodes();var E=i.getCachedEdges();
var J=i.dragData.possibleDragElements;var Q=i.hoverData.capture;if(!Q){var P=i.findContainerPageCoords();if(I.pageX>P[0]&&I.pageX<P[0]+i.data.container.clientWidth&&I.pageY>P[1]&&I.pageY<P[1]+i.data.container.clientHeight){}else{return}}var B=new s.Event(I,{type:"mousemove"});if(H!=null){H.trigger(B)}else{if(H==null){D.trigger(B)}}if(i.hoverData.dragging){D.panBy({x:O[0]*D.zoom(),y:O[1]*D.zoom()});K=i.projectIntoViewport(I.pageX,I.pageY)}else{if(L[4]==1&&M==null&&(new Date()).getTime()-i.hoverData.downTime>200&&(Math.abs(L[3]-L[1])+Math.abs(L[2]-L[0])<4)){i.hoverData.dragging=true;
L[4]=0}else{if(H!=N){if(N){N.trigger(new s.Event(I,{type:"mouseout"}))}if(H){H.trigger(new s.Event(I,{type:"mouseover"}))}i.hoverData.last=H}if(M){var G=new s.Event(I,{type:"position"});for(var F=0;F<J.length;F++){if(!J[F]._private.locked&&J[F]._private.group=="nodes"){J[F]._private.position.x+=O[0];J[F]._private.position.y+=O[1];J[F].trigger(G)}}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Nodes dragged")}i.data.canvasNeedsRedraw[w]=true;i.data.canvasRedrawReason[w].push("Mouse moved, redraw selection box")
}}L[2]=K[0];L[3]=K[1];i.redraw()},false);window.addEventListener("mouseup",function(S){var N=i.hoverData.capture;if(!N){return}i.hoverData.capture=false;var D=i.data.cy;var H=i.projectIntoViewport(S.pageX,S.pageY);var L=i.data.select;var T=i.findNearestElement(H[0],H[1]);var M=i.getCachedNodes();var C=i.getCachedEdges();var G=i.dragData.possibleDragElements;var J=i.hoverData.down;if(T==null||T!=J||!T.selected()){var F=new s.Event(S,{type:"unselect"});for(var Q=0;Q<G.length;Q++){if(G[Q]._private.selected){G[Q]._private.selected=false;
G[Q].trigger(F);G[Q].updateStyle(false)}}if(G.length>0){i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("De-select")}G=i.dragData.possibleDragElements=[]}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)==0){var B=new s.Event(S,{type:"click"});if(T!=null){T.trigger(B)}else{if(T==null){D.trigger(B)}}}var E=new s.Event(S,{type:"mouseup"});if(T!=null){T.trigger(E)}else{if(T==null){D.trigger(E)}}if(T==J&&(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)<7)){if(T!=null&&T._private.selectable&&T._private.selected==false){T._private.selected=true;
T.trigger(new s.Event(S,{type:"select"}));T.updateStyle(false);G.push(T);i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("sglslct")}}else{if(T==J){if(T!=null&&T._private.grabbed){var R=new s.Event(S,{type:"free"});T._private.grabbed=false;T.trigger(R);var K=T._private.edges;for(var P=0;P<K.length;P++){K[P]._private.grabbed=false}}}}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)>7&&L[4]){var I=i.getAllInBox(L[0],L[1],L[2],L[3]);var O=new s.Event(S,{type:"select"});for(var Q=0;Q<I.length;
Q++){if(I[Q]._private.selectable){I[Q]._private.selected=true;I[Q].trigger(O);I[Q].updateStyle(false);G.push(I[Q])}}if(I.length>0){i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Selection")}}i.hoverData.dragging=false;if(!L[4]){var R=new s.Event(S,{type:"free"});for(var Q=0;Q<G.length;Q++){G[Q]._private.grabbed=false;if(G[Q]._private.group=="nodes"){var K=G[Q]._private.edges;for(var P=0;P<K.length;P++){K[P]._private.grabbed=false}}G[Q].trigger(R)}i.data.canvasNeedsRedraw[d]=true;
i.data.canvasRedrawReason[d].push("Node/nodes back from drag");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Node/nodes back from drag")}L[4]=0;i.hoverData.down=null;i.data.canvasNeedsRedraw[w]=true;i.data.canvasRedrawReason[w].push("Mouse up, selection box gone");i.redraw()},false);var A=function(D){var F=i.data.cy;var E=i.projectIntoViewport(D.pageX,D.pageY);var C=[E[0]*F.zoom()+F.pan().x,E[1]*F.zoom()+F.pan().y];if(i.zoomData.freeToZoom){D.preventDefault();var B=D.wheelDeltaY/1000||D.detail/-8.4;
F.zoom({level:F.zoom()*Math.pow(10,B),position:{x:C[0],y:C[1]}})}};i.data.container.addEventListener("mousewheel",A,false);i.data.container.addEventListener("DOMMouseScroll",A,false);i.data.container.addEventListener("mousemove",function(B){if(i.zoomData.lastPointerX&&i.zoomData.lastPointerX!=B.pageX&&!i.zoomData.freeToZoom){i.zoomData.freeToZoom=true}i.zoomData.lastPointerX=B.pageX},false);i.data.container.addEventListener("mouseout",function(B){i.zoomData.freeToZoom=false;i.zoomData.lastPointerX=null
},false);i.data.container.addEventListener("mouseout",function(B){cy.trigger(new s.Event(B,{type:"mouseout"}))},false);i.data.container.addEventListener("mouseover",function(B){cy.trigger(new s.Event(B,{type:"mouseover"}))},false);i.data.container.addEventListener("touchstart",function(K){K.preventDefault();i.touchData.capture=true;var D=i.data.cy;var B=i.getCachedNodes();var G=i.getCachedEdges();var C=i.touchData.now;var E=i.touchData.earlier;if(K.touches[0]){var M=i.projectIntoViewport(K.touches[0].pageX,K.touches[0].pageY);
C[0]=M[0];C[1]=M[1]}if(K.touches[1]){var M=i.projectIntoViewport(K.touches[1].pageX,K.touches[1].pageY);C[2]=M[0];C[3]=M[1]}if(K.touches[2]){var M=i.projectIntoViewport(K.touches[2].pageX,K.touches[2].pageY);C[4]=M[0];C[5]=M[1]}if(K.touches[2]){}else{if(K.touches[1]){}else{if(K.touches[0]){var J=i.findNearestElement(C[0],C[1]);if(J!=null){i.touchData.start=J;if(J._private.group=="nodes"){J._private.grabbed=true;J.trigger(new s.Event(K,{type:"grab"}));i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node start");
i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("touchdrag node start");var I=J._private.edges;for(var F=0;F<I.length;F++){I[F]._private.grabbed=true}}J.trigger(new s.Event(K,{type:"touchstart"}))}else{if(J==null){D.trigger(new s.Event(K,{type:"touchstart"}))}}for(var H=0;H<C.length;H++){E[H]=C[H];i.touchData.startPosition[H]=C[H]}i.touchData.singleTouchMoved=false;i.touchData.singleTouchStartTime=l();var L=setTimeout(function(){if(i.touchData.singleTouchMoved==false&&l()-i.touchData.singleTouchStartTime<1040&&l()-i.touchData.singleTouchStartTime>960){if(i.touchData.start){i.touchData.start.trigger(new s.Event(K,{type:"taphold"}))
}else{i.data.cy.trigger(new s.Event(K,{type:"taphold"}))}}},1000)}}}i.redraw()},true);window.addEventListener("touchmove",function(N){var R=i.touchData.capture;if(!R){return}N.preventDefault();var F=i.data.cy;var B=i.getCachedNodes();var I=i.getCachedEdges();var D=i.touchData.now;var G=i.touchData.earlier;if(N.touches[0]){var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);D[0]=P[0];D[1]=P[1]}if(N.touches[1]){var P=i.projectIntoViewport(N.touches[1].pageX,N.touches[1].pageY);D[2]=P[0];
D[3]=P[1]}if(N.touches[2]){var P=i.projectIntoViewport(N.touches[2].pageX,N.touches[2].pageY);D[4]=P[0];D[5]=P[1]}var Q=[];for(var H=0;H<D.length;H++){Q[H]=D[H]-G[H]}if(N.touches[2]){}else{if(N.touches[1]){var M=[(Q[0]+Q[2])/2,(Q[1]+Q[3])/2];F.panBy({x:M[0]*F.zoom(),y:M[1]*F.zoom()});var K=Math.sqrt(Math.pow(G[2]-G[0],2)+Math.pow(G[3]-G[1],2));var E=Math.sqrt(Math.pow(D[2]-D[0],2)+Math.pow(D[3]-D[1],2));var O=E/K;if(O>1){O=(O-1)*1.5+1}else{O=1-(1-O)*1.5}F.zoom({level:F.zoom()*O,position:{x:(D[0]+D[2])/2,y:(D[1]+D[3])/2}});
if(N.touches[0]){var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);D[0]=P[0];D[1]=P[1]}if(N.touches[1]){var P=i.projectIntoViewport(N.touches[1].pageX,N.touches[1].pageY);D[2]=P[0];D[3]=P[1]}if(N.touches[2]){var P=i.projectIntoViewport(N.touches[2].pageX,N.touches[2].pageY);D[4]=P[0];D[5]=P[1]}}else{if(N.touches[0]){var C=i.touchData.start;if(C!=null&&C._private.group=="nodes"){C._private.position.x+=Q[0];C._private.position.y+=Q[1];i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node");
C.trigger(new s.Event(N,{type:"position"}))}if(C!=null){C.trigger(new s.Event(N,{type:"touchmove"}))}if(C==null){var L=i.findNearestElement(D[0],D[1]);if(L!=null){L.trigger(new s.Event(N,{type:"touchmove"}))}if(L==null){F.trigger(new s.Event(N,{type:"touchmove"}))}}for(var J=0;J<D.length;J++){if(D[J]&&i.touchData.startPosition[J]&&Math.abs(D[J]-i.touchData.startPosition[J])>4){i.touchData.singleTouchMoved=true}}if(C==null){F.panBy({x:Q[0]*F.zoom(),y:Q[1]*F.zoom()});var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);
D[0]=P[0];D[1]=P[1]}}}}for(var H=0;H<D.length;H++){G[H]=D[H]}i.redraw()},true);window.addEventListener("touchend",function(K){var M=i.touchData.capture;if(!M){return}i.touchData.capture=false;K.preventDefault();var E=i.data.cy;var B=i.getCachedNodes();var H=i.getCachedEdges();var D=i.touchData.now;var F=i.touchData.earlier;if(K.touches[0]){var L=i.projectIntoViewport(K.touches[0].pageX,K.touches[0].pageY);D[0]=L[0];D[1]=L[1]}if(K.touches[1]){var L=i.projectIntoViewport(K.touches[1].pageX,K.touches[1].pageY);
D[2]=L[0];D[3]=L[1]}if(K.touches[2]){var L=i.projectIntoViewport(K.touches[2].pageX,K.touches[2].pageY);D[4]=L[0];D[5]=L[1]}if(K.touches[2]){}else{if(K.touches[1]){}else{if(K.touches[0]){}else{if(!K.touches[0]){var C=i.touchData.start;if(C!=null){if(C._private.grabbed==true){C._private.grabbed=false;C.trigger(new s.Event(K,{type:"free"}))}var I=C._private.edges;for(var G=0;G<I.length;G++){I[G]._private.grabbed=false}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node end");
i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("touchdrag node end");C.trigger(new s.Event(K,{type:"touchend"}));i.touchData.start=null}else{var J=i.findNearestElement(D[0],D[1]);if(J!=null){J.trigger(new s.Event(K,{type:"touchend"}))}if(J==null){E.trigger(new s.Event(K,{type:"touchend"}))}}if(i.touchData.singleTouchMoved==false){if(C!=null&&C._private.selectable&&C._private.selected==false){E.elements(":selected").unselect();C._private.selected=true;C.trigger(new s.Event(K,{type:"select"}));
C.updateStyle(false);i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("sglslct")}if(C){C.trigger(new s.Event(K,{type:"tap"}))}else{E.trigger(new s.Event(K,{type:"tap"}))}}i.touchData.singleTouchMoved=true}}}}for(var G=0;G<D.length;G++){F[G]=D[G]}i.redraw()},true)};t.prototype.init=function(){};t.prototype.getCachedNodes=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedNodes==undefined){i.cache.cachedNodes=A.nodes()}return i.cache.cachedNodes
};t.prototype.updateNodesCache=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedNodes=A.nodes()};t.prototype.getCachedEdges=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedEdges==undefined){i.cache.cachedEdges=A.edges()}return i.cache.cachedEdges};t.prototype.updateEdgesCache=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedEdges=A.edges()};t.prototype.projectIntoViewport=function(C,A){var i,G;
var D=0;var B=0;var F;F=this.data.container;var E=false;while(F!=null){if(typeof(F.offsetLeft)=="number"){D+=F.offsetLeft;B+=F.offsetTop;if(F==document.body||F==document.header){E=true}if(!E){D-=F.scrollLeft;B-=F.scrollTop}}F=F.offsetParent}i=C-D;G=A-B;i-=this.data.cy.pan().x;G-=this.data.cy.pan().y;i/=this.data.cy.zoom();G/=this.data.cy.zoom();return[i,G]};t.prototype.findContainerPageCoords=function(){var i,E;var B=0;var A=0;var D;D=this.data.container;var C=false;while(D!=null){if(typeof(D.offsetLeft)=="number"){B+=D.offsetLeft;
A+=D.offsetTop;if(D==document.body||D==document.header){C=true}if(!C){B-=D.scrollLeft;A-=D.scrollTop}}D=D.parentNode}return[B,A]};t.prototype.findNearestElement=function(A,G){var F=this.data;var C=this.getCachedNodes();var B=this.getCachedEdges();var E=[];for(var D=0;D<C.length;D++){if(n[C[D]._private.style.shape.value].checkPointRough(A,G,C[D]._private.style["border-width"].value,C[D]._private.style.width.value,C[D]._private.style.height.value,C[D]._private.position.x,C[D]._private.position.y)&&n[C[D]._private.style.shape.value].checkPoint(A,G,C[D]._private.style["border-width"].value,C[D]._private.style.width.value/2,C[D]._private.style.height.value/2,C[D]._private.position.x,C[D]._private.position.y)){E.push(C[D])
...
A.sort(function(L,K){var J=L._private.style["z-index"].value-K._private.style["z-index"].value;if(J==0){if(L._private.group=="nodes"&&K._private.group=="edges"){return 1}else{if(L._private.group=="edges"&&K._private.group=="nodes"){return -1}}}return 0})}if(F.canvasNeedsRedraw[z]){var C=F.canvases[4].getContext("2d");C.setTransform(1,0,0,1,0,0);C.clearRect(0,0,C.canvas.width,C.canvas.height);C.translate(D.pan().x,D.pan().y);C.scale(D.zoom(),D.zoom());var G;for(var H=0;H<A.length;H++){G=A[H];if(!G._private.grabbed){if(G._private.group=="nodes"){this.drawNode(C,G)
}else{if(G._private.group=="edges"){this.drawEdge(C,G)}}}}for(var H=0;H<A.length;H++){G=A[H];if(!G._private.grabbed){if(G._private.group=="nodes"){this.drawNodeText(C,G)}else{if(G._private.group=="edges"){this.drawEdgeText(C,G)}}}}F.canvasNeedsRedraw[z]=false;F.canvasRedrawReason[z]=[]}if(F.canvasNeedsRedraw[d]){var C=F.canvases[2].getContext("2d");C.setTransform(1,0,0,1,0,0);C.clearRect(0,0,C.canvas.width,C.canvas.height);C.translate(D.pan().x,D.pan().y);C.scale(D.zoom(),D.zoom());var G;for(var H=0;
H<A.length;H++){G=A[H];if(G._private.grabbed){if(G._private.group=="nodes"){this.drawNode(C,G)}else{if(G._private.group=="edges"){this.drawEdge(C,G)}}}}for(var H=0;H<A.length;H++){G=A[H];if(G._private.grabbed){if(G._private.group=="nodes"){this.drawNodeText(C,G)}else{if(G._private.group=="edges"){this.drawEdgeText(C,G)}}}}F.canvasNeedsRedraw[d]=false;F.canvasRedrawReason[d]=[]}if(F.canvasNeedsRedraw[w]){var C=F.canvases[0].getContext("2d");C.setTransform(1,0,0,1,0,0);C.clearRect(0,0,C.canvas.width,C.canvas.height);
C.translate(D.pan().x,D.pan().y);C.scale(D.zoom(),D.zoom());if(F.select[4]==1){var I=D.style()._private.coreStyle;var i=I["selection-box-border-width"].value/F.cy.zoom();C.lineWidth=i;C.fillStyle="rgba("+I["selection-box-color"].value[0]+","+I["selection-box-color"].value[1]+","+I["selection-box-color"].value[2]+","+I["selection-box-opacity"].value+")";C.fillRect(F.select[0],F.select[1],F.select[2]-F.select[0],F.select[3]-F.select[1]);if(i>0){C.strokeStyle="rgba("+I["selection-box-border-color"].value[0]+","+I["selection-box-border-color"].value[1]+","+I["selection-box-border-color"].value[2]+","+I["selection-box-opacity"].value+")";
