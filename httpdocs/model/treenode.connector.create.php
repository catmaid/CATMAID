...
if ( ! $input_id ) {
    echo makeJSON( array( 'error' => 'You need to provide a valid input treenode id.' ) );
    return;
}

// the locatoin id might be not existing, then create a new connector instance
...
if(!$it_id) {
    echo makeJSON( array( 'error' => 'Can not find "'.$input_type.'" class for this project' ) );
    return;
}

$lt_id = $db->getClassId( $pid, $location_type );
if(!$lt_id) {
    echo makeJSON( array( 'error' => 'Can not find "'.$location_type.'" class for this project' ) );
    return;
}
...
if(!$ir_id) {
    echo makeJSON( array( 'error' => 'Can not find "'.$input_relation.'" relation for this project' ) );
    return;
}

$ilr_id = $db->getRelationId( $pid, $input_location_relation );
if(!$ilr_id) {
    echo makeJSON( array( 'error' => 'Can not find "'.$input_location_relation.'" relation for this project' ) );
    return;
}

$lr_id = $db->getRelationId( $pid, $location_relation );
if(!$lr_id) {
    echo makeJSON( array( 'error' => 'Can not find "'.$location_relation.'" relation for this project' ) );
    return;
}
