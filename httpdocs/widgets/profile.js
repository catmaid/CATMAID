...
)
{
	...
	)
	{
		...
		case CK_CORNER:
			type = t;
			self.before = { x : self.key.x, y : self.key.y };
			self.after = { x : self.key.x, y : self.key.y };
			break;
		case CK_CUSP:
			type = t;
			if ( self.before.x == self.key.x && self.before.y == self.key.y )
				if ( kb )
				{
					self.before.x = self.key.x - ( self.key.x - kb.after.x ) / 2;
					self.before.y = self.key.y - ( self.key.y - kb.after.y ) / 2;
				}
			if ( self.after.x == self.key.x && self.after.y == self.key.y )
				if ( ka )
				{
		...
		case CK_SYMMETRIC:
			...
			if ( kb && ka )
			{
				var dx = ( ka.x - kb.x ) / 2;
				var dy = ( ka.y - kb.y ) / 2;
				self.before = { x : self.key.x - dx, y : self.key.y - dy };
				self.after = { x : self.key.x + dx, y : self.key.y + dy };
			}
			break;
		}
		return;
...
function Profile()
{
	...
	this.clearCanvas = function()
	{
		var ctx = getContext();
		if ( ctx )
			ctx.clearRect( 0, 0, max.x - min.x, max.y - min.y );
		return;
	}
	...
	this.draw = function()
	{
		...
		if ( ctx )
		{
			...
			ctx.beginPath();
			ctx.moveTo(
				screen.scale * ( keys[ 0 ].key.x - min.x ),
				screen.scale * ( keys[ 0 ].key.y - min.y ) );
			var i;
			var n = keys.length;
			for ( i = 1; i < n; ++i )
			{
			...
			ctx.bezierCurveTo(
					screen.scale * ( keys[ --i ].after.x - min.x ),
					screen.scale * ( keys[ i ].after.y - min.y ),
					screen.scale * ( keys[ 0 ].before.x - min.x ),
					screen.scale * ( keys[ 0 ].before.y - min.y ),
					screen.scale * ( keys[ 0 ].key.x - min.x ),
					screen.scale * ( keys[ 0 ].key.y - min.y ) );
			ctx.fill();
		}
	...
	this.isVisible = function()
	{
		return (
			max.x >= screen.x &&
			max.y >= screen.y &&
			min.x <= screen.x + screen.width &&
			min.y <= screen.y + screen.height );
	}
