...
class CreateUserProfilesForDjangoAuthUsers {
    var $name = 'Create user profiles for users in auth_user table';

    function apply( $db, $ignoreErrors) {
        try {
            error_log("Running the migration: ".$this->name);
            $db->getResult("SAVEPOINT create_userprofiles");
            # Check if the userprofile table already exists

            # Add an entry for each user if it isn't present already
            foreach( $db->getResult('SELECT * FROM "auth_user"') as $u ) {
                $user_id = $u['id'];
                $userprofile = $db->getResult("SELECT * FROM catmaid_userprofile
                    WHERE user_id = ".$user_id);
                if ($userprofile === FALSE) {
                    error_log("The table catmaid_userprofiles might not exist. Have you run ./manage.py syncdb?");
                } elseif (sizeof($userprofile) > 0) {
                    error_log("User ".$user_id." has already a user profile.");
                } else {
                    $db->getResult("INSERT INTO catmaid_userprofile (user_id,
                         inverse_mouse_wheel) VALUES (".$user_id.", FALSE)");
                }
            }
		} catch( Exception $e ) {
			if ($ignoreErrors) {
				error_log("Ignoring the failed migration: ".$e);
				$db->getResult("ROLLBACK TO SAVEPOINT create_userprofiles");
			} else {
				error_log("The migration failed: ".$e);
				throw $e;
			}
