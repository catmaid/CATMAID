...
with closing(h5py.File(op.join(filepath, '%s_%s.hdf' % (project_id, stack_id)), 'w')) as hfile:
    ...
    scale0.create_dataset('data', data=np.random.random_integers(0,255,
            (1024,1024,10)), dtype = np.uint8)

    scale1=scale.create_group('1')
    scale1.create_dataset('data', data=np.random.random_integers(0,255,
            (512,512,10)), dtype = np.uint8)

    scale2=scale.create_group('2')
    scale2.create_dataset('data', data=np.random.random_integers(0,255,
            (256,256,10)), dtype = np.uint8)
